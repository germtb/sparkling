...
module.exports = {
	...
	activate() {
		...
		const files = () => {
			const options = filesFactory(React, store)
			sparkling(options)
		}

		const gitFiles = () => {
			const options = gitFilesFactory(React, store)
			sparkling(options)
		}

		const gitBranches = () => {
			const options = gitBranchesFactory(React, store)
			sparkling(options)
		}

		const lines = () => {
			const options = linesFactory(React, store)
			sparkling(options)
		}
