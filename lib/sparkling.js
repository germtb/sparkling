...
Rx.Observable.combineLatest(fromSelector(getData), fromSelector(getPattern))
	...
	.subscribe(([data, pattern]) => {
		if (!pattern.length) {
			...
			if (fuzzySortProcess) {
				fuzzySortProcess.kill()
			}

			fuzzySortProcess = fork(`${__dirname}/../tasks/asyncFuzzySearch`, [data])
			fuzzySortProcess.on('message', data => {
				store.dispatch({
					type: 'SET_FILTERED_DATA',
					payload: { data }
				})
