...
const sparkling = optionsFactory => {
	const {
		loadData,
		accept,
...
module.exports = {
	...
	activate() {
		...
		const gitFiles = sparkling(gitFilesFactory)
		const files = sparkling(filesFactory)
		const gitBranches = sparkling(gitBranchesFactory)
		const lines = sparkling(linesFactory)
