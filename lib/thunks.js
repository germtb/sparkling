...
export const next: Thunk<void> = () => (dispatch, getState) => {
...
export const previous: Thunk<void> = () => (dispatch, getState) => {
	const state = getState()
	const index = getIndex(state)
...
export const left: Thunk<void> = () => (dispatch, getState) => {
	const state = getState()
	const index = getIndex(state)
	const options = getOptions(state)
...
export const right: Thunk<void> = () => (dispatch, getState) => {
	const state = getState()
	const index = getIndex(state)
	const sparklingData = getSparklingData(state)
	const options = getOptions(state)
...
export const hide: Thunk<void> = () => dispatch => {
	dispatch({ type: 'HIDE' })
}

export const accept: Thunk<void> = () => (dispatch, getState) => {
	const state = getState()
...
export const findToggle: Thunk<{ find: string, scope: string }> = ({
	find,
	scope
} = {}) => (dispatch, getState) => {
...
export const copyFilesConfirm: Thunk<() => void> = onDone => (
	dispatch,
	getState
) => {
...
export const moveFilesConfirm: Thunk<() => void> = onDone => (
	dispatch,
	getState
) => {
	const extraInput = getExtraInput(getState())
