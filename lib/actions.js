...
import { SLICE_LENGTH, ROWS } from './constants'
...
export const next = () => {
	...
	if (index === SLICE_LENGTH - 1) {
		...
		const value = Math.min(offset + 1, sparklingData.length - SLICE_LENGTH)
...
export const right = () => {
	...
	if (index === SLICE_LENGTH - 1) {
		const offset = getOffset(state)
		const value = Math.min(offset + ROWS, sparklingData.length - SLICE_LENGTH)
		store.dispatch({ type: 'SET_OFFSET', payload: { value } })
	} else {
		const value = Math.min(
			index + ROWS,
